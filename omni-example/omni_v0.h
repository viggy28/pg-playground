...
#else
#error                                                                         \
    "Ensure this version of Postgres is compatible with the above definition or use a new one"
#endif
...
#define OMNI_MODULE_INFO(...)                                                  \
  omni_module_information _omni_module_information = {__VA_ARGS__}
...
typedef struct {
  ...
  uint8_t revision; // version's backward compatible revision (number, but shown
                    // as letter 'A','B' to distinguish for Major.Minor. Version
                    // 0 allows for breaking revisions.
} omni_magic;

StaticAssertDecl(sizeof(omni_magic) <= UINT16_MAX,
                 "omni_magic should fit into 16 bits");
...
/**
 * @brief Every dynamic extension should use this macro in addition to
 * `PG_MODULE_MAGIC`
 *
 */
#define OMNI_MAGIC                                                             \
 ...
 * @param allocated True when the area was allocated in this call, false if it
 * was found
 */
typedef void (*omni_allocate_shmem_callback_function)(const omni_handle *handle,
                                                      void *ptr, void *data,
                                                      bool allocated);
/**
 ...
 *
 */
typedef void *(*omni_allocate_shmem_function)(
    const omni_handle *handle, const char *name, size_t size,
 ...
 * @param found Pointer to a flag indicating if the allocation was found
 */
typedef void (*omni_deallocate_shmem_function)(const omni_handle *handle,
 ...
 */
typedef void *(*omni_lookup_shmem_function)(const omni_handle *handle,
                                            const char *name, bool *found);
 ...
 *
 * @param handle Handle passed by the loader
 * @param name Name of the lock. Must be static or allocated under a top memory
 * context
 */
typedef void (*omni_register_lwlock_function)(const omni_handle *handle,
                                              LWLock *lock, const char *name,
                                              bool initialize);

/**
 * @brief Unregister a lock
 *
 * This will allow to reclaim tranche ID for future use (subject to loader's
 * support and algorithm)
 *
 * Would be typically called from `_Omni_unload` to ensure it is called once,
 * when all backends have deinitialized.
 */
typedef void (*omni_unregister_lwlock_function)(const omni_handle *handle,
                                                LWLock *lock);

typedef struct omni_handle omni_handle;

typedef enum {
  hook_next_action_finish = 0,
  hook_next_action_next = 1
} hook_next_action;
typedef union {
  bool bool_value;
  char *char_value;
...
typedef void (*omni_hook_emit_log_t)(omni_hook_handle *handle,
                                     ErrorData *edata);
typedef void (*omni_hook_check_password_t)(
    omni_hook_handle *handle, const char *username, const char *shadow_pass,
...
typedef void (*omni_hook_executor_start_t)(omni_hook_handle *handle,
                                           QueryDesc *queryDesc, int eflags);
typedef void (*omni_hook_executor_run_t)(omni_hook_handle *handle,
                                         QueryDesc *queryDesc,
                                         ScanDirection direction, uint64 count,
                                         bool execute_once);
typedef void (*omni_hook_executor_finish_t)(omni_hook_handle *handle,
                                            QueryDesc *queryDesc);
typedef void (*omni_hook_executor_end_t)(omni_hook_handle *handle,
                                         QueryDesc *queryDesc);
typedef void (*omni_hook_process_utility_t)(
    omni_hook_handle *handle, PlannedStmt *pstmt, const char *queryString,
    bool readOnlyTree, ProcessUtilityContext context, ParamListInfo params,
    QueryEnvironment *queryEnv, DestReceiver *dest, QueryCompletion *qc);
typedef void (*omni_hook_xact_callback_t)(omni_hook_handle *handle,
                                          XactEvent event);
typedef void (*omni_hook_subxact_callback_t)(omni_hook_handle *handle,
                                             SubXactEvent event);
...
// Assigned values MUST be consequtive, linear and start from 0 for
// __OMNI_HOOK_TYPE_COUNT to be correct
typedef enum {
 ...
 *
 * Best place to register the hook is `_Omni_init`, they hook will be registered
 * in every backend.
 *
 * If done in `_Omni_load`, it will be only registered in the backend that
 * caused the module to be loaded.
 *
 * If done during a regular function call, the backend will be only registered
 * in the backend where it was called.
 *
 * @param handle
 * @param hook Hook to register. `hook` doesn't need to be allocated in a memory
 * context, but `hook->name` must be allocated either statically or in a context
 * that outlives the module (either the module's own memory context, or
 * something that would live longer than that)
 */
typedef void (*omni_register_hook_function)(const omni_handle *handle,
                                            omni_hook *hook);
...
StaticAssertDecl(
    offsetof(omni_guc_int_variable, value) == 0,
...
StaticAssertDecl(
    offsetof(omni_guc_bool_variable, value) == 0,
...
StaticAssertDecl(
    offsetof(omni_guc_real_variable, value) == 0,
...
StaticAssertDecl(
    offsetof(omni_guc_string_variable, value) == 0,
...
StaticAssertDecl(
    offsetof(omni_guc_enum_variable, value) == 0,
    "to ensure casting to (int *) will get us straight to the pointer");
...
typedef void (*omni_request_bgworker_start_function)(
    const omni_handle *handle, BackgroundWorker *bgworker,
    omni_bgworker_handle *bgworker_handle, const omni_bgworker_options options);

typedef void (*omni_request_bgworker_termination_function)(
    const omni_handle *handle, omni_bgworker_handle *bgworker_handle,
 ...
 *
 * Switches are simple 1/0 values that can be atomically switched either to `on`
 * or `off` state. This function returns a number that sets bits that were
 * flipped by this operation to `1`. This allows the caller to be able to
 * determine if they were the first party to flip the switch, enabling a
 * primitive form of leader election.
 *
 * @param handle
 * @param op `omni_switch_on` turns the switches on, `omni_switch_off` turns the
 * switches off
 * @param switchboard switchboard ID
 *        Omni provides 64 switches per switchboard. At least one switchboard
 * must be provided
 *        (`switchboard = 0`) by the loader, attempts to get `switchboard`
 * greater than zero, may trigger `ERROR` if no more switchboards can be
 * allocated.
 * @param mask select switches to perform `op` on
 *
 */
typedef uint64 (*omni_atomic_switch_function)(const omni_handle *handle,
                                              omni_switch_operation op,
